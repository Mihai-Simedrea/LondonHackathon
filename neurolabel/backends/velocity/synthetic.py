...
def generate_synthetic_game(duration_seconds, base_time=None, record_interval_frames=None):
    """
    Generate synthetic game recording using heuristic AI with OC-correlated noise.

    By default records once per second (public/test-friendly behavior).
    Callers can request higher-frequency capture via `record_interval_frames`.

    Records at ~10 Hz (every 6 frames) when `record_interval_frames=6`.
    ...
    """
    record_interval = FPS if record_interval_frames is None else int(record_interval_frames)
    if record_interval <= 0:
        raise ValueError("record_interval_frames must be > 0")
    ...
    for frame in range(total_frames):
        ...
        # Record game state at the requested cadence.
...
def generate_synthetic(duration_seconds=600):
    ...
    game_records = generate_synthetic_game(
        duration_seconds,
        base_time=base_time,
